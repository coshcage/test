...
int cbftvstemp(void * pitem, size_t param)
{
	P_VERTEX_L pvtx = (P_VERTEX_L)pitem;
	P_NFAELE pn = (P_NFAELE)pvtx->vid;
	printf("%zd---------------\n", pn->id);
	PrintParrList(pn->parrBNFLst);
	DWC4100(param);
	return CBF_CONTINUE;
}

int cbftvstemp2(void * pitem, size_t param)
{
	P_VERTEX_L pvtx = (P_VERTEX_L)pitem;
	P_NFAELE pn = (P_NFAELE)pvtx->vid;
	printf("%zd---------------\n", pn->id);
